...
class GBMEnvironment:
    ...
    def __init__(self, utility_func, num_of_days=1826, window_size=100, eval=False) -> None:
    ...
    def reset(self):
        ...
        factor = min(self.i, 100000) / 100000 if not self.eval else 1
        cov = cov * factor
        # generate the GBM
    ...
    def step(self, action):
        ...
        # reward
        r = self.func(nxt_amount)*GAMMA - self.func(amount)
        ...
        with torch.no_grad():
            if (step+1)%log_period == 0:
                ...
                print('step', '%6d'%(step+1), 'reward', '%3.2f'%reward, 
                    'terminal net value', '%3.2f'%obs_[-1][-2], 'benchmark', '%3.2f'%max(obs_[-1][:2]),
                    'min', '%3.2f'%min(obs_[-1][:2]),
...
q_optimizer = torch.optim.Adam(qnet.parameters(), lr=1e-4)
actor.initialize()
qnet.initialize()
train(actor, qnet, actor_optimizer, q_optimizer, max_steps=2000000, max_buffer_size=10000,
    save_dir=f'results/ddpg', update_time=4, update_batch=1, log_period=1000,
    batch_size=16, start_after_steps=10000, polyak=0.995, gamma=1,
    func=u_func, window_size=100, save_period=10000)
